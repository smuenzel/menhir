...
let rec run mode s =
  ...
  if must_read_positions then begin
    prim startp (PrimOCamlPureFunctionCall ("Lexing.lexeme_start_p", [ VReg lexbuf ]));
    prim endp (PrimOCamlPureFunctionCall ("Lexing.lexeme_end_p", [ VReg lexbuf ]));
  end;
  ...
  if Lr1.is_start s then begin
    prim endp (PrimOCamlPureFunctionCall ("Lexing.lexeme_start_p", [ VReg lexbuf ]));
    move initp endp
  end;
